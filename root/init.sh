#!/bin/bash
		...
		if [[ -n "${PHP_SENDMAIL_PATH+x}" ]]; then
		    echo "php_admin_value[sendmail_path] = ${PHP_SENDMAIL_PATH}" >> /config/php/fpm/pool.d/application.conf
               ...
               #install configs into container, match cli and fpm configs

                mkdir /run/php
#		cp /config/php/php.ini /etc/php/7.4/fpm/conf.d/container-php.ini
#		cp /config/php/php.ini /etc/php/7.4/cli/conf.d/container-php.ini
...
if [[ -e /config/httpd/conf.d/mods.enable ]]; then
  while read MODULE_ENABLE
    do
       if [[ -n "$MODULE_ENABLE" ]]; then
         a2enmod $MODULE_ENABLE
       fi
    done < /config/httpd/conf.d/mods.enable
fi

#while read MODULE_DISABLE
#  do
#   a2dismod $MODULE_DISABLE
#  done < /config/httpd/conf.d/mods.disable
